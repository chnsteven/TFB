...
read -r -a UCDGPT_GPU_ARGS <<< "$TFB_GPU_DEVICES"
TFB_SERIAL_RUN_FLAGS+=(--gpus "${UCDGPT_GPU_ARGS[@]}")
for i in "${!TFB_SERIAL_RUN_FLAGS[@]}"; do
  if [[ "${TFB_SERIAL_RUN_FLAGS[$i]}" == "--num-workers" ]]; then
    TFB_SERIAL_RUN_FLAGS[$((i + 1))]=2
  fi
done
